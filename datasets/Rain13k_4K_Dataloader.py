...
class TrainData(data.Dataset):
    def __init__(self, crop_size, train_data_dir, only_h_flip=False):
        super().__init__()
        train_list_rain = '/home/jxy/projects_dir/datasets/4K-Rain13k/Train.txt'
    ...
    def get_images(self, index):
        ...
        # weight_map = pixel_weight(H, W, 256, 100)
        ...
        # weight_map = img2tensor(weight_map.copy(), bgr2rgb=False, float32=True)
...
class TestData(data.Dataset):
    def __init__(self, local_size, val_data_dir, flag=True):
        ...
        val_list_rain = "/home/jxy/projects_dir/datasets/4K-Rain13k/Test.txt"
...
class TestData_Whole(data.Dataset):
    def __init__(self, local_size, val_data_dir, flag=True):
        super().__init__()
...
class Test(data.Dataset):
    ...
    def get_images(self, index):
        ...
        rain_path = os.path.join(self.val_data_dir, 'UDR-Mixer', rain_name)
        gt_path = os.path.join(self.val_data_dir, 'test', 'target', gt_name)
        rain_img = imfrombytes(read_img255(rain_path), float32=True)
        gt_img = imfrombytes(read_img255(gt_path), float32=True)
        # h, w, c = rain_img.shape
        # h = h - h % self.local_size
