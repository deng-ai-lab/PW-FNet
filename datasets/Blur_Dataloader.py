...
class TrainData_DPDD_dual(data.Dataset):
    def __init__(self, crop_size, train_data_dir, only_h_flip=False):
        super().__init__()
        train_list_rain = '/home/jxy/projects_dir/datasets/DPDD/train.txt'
    ...
    def get_images(self, index):
        rain_name = self.rain_names[index]
        gt_name = self.gt_names[index]
        rain_path_r = os.path.join(self.train_data_dir, 'inputR_crops', rain_name)
        rain_path_l = os.path.join(self.train_data_dir, 'inputL_crops', rain_name)
        gt_path = os.path.join(self.train_data_dir, 'target_crops', gt_name)
        rain_img_r = imfrombytes(read_img255(rain_path_r), float32=True)
        rain_img_l = imfrombytes(read_img255(rain_path_l), float32=True)
        gt_img = imfrombytes(read_img255(gt_path), float32=True)
        [rain_img_r, rain_img_l, gt_img] = augment([rain_img_r, rain_img_l, gt_img], size=self.crop_size, edge_decay=0.,
                                     only_h_flip=self.only_h_flip)

        rain_r = img2tensor(rain_img_r, bgr2rgb=True, float32=True)
        rain_l = img2tensor(rain_img_l, bgr2rgb=True, float32=True)
        rain = torch.cat([rain_l, rain_r], dim=0)
        gt = img2tensor(gt_img, bgr2rgb=True, float32=True)
...
class TrainData_DPDD_New(data.Dataset):
    def __init__(self, crop_size, train_data_dir, only_h_flip=False):
        super().__init__()
        train_list_rain = '/home/jxy/projects_dir/datasets/DPDD_new/train.txt'
    ...
    def get_images(self, index):
        rain_name = self.rain_names[index]
        gt_name = self.gt_names[index]
        rain_path = os.path.join(self.train_data_dir, 'inputC', rain_name)
...
class TestRealBlurR(data.Dataset):
    ...
    def get_images(self, index):
        ...
        rain_img = imfrombytes(read_img255(rain_path), float32=True)
        gt_img = imfrombytes(read_img255(gt_path), float32=True)
        [rain_img, gt_img] = align([rain_img, gt_img], size_H=736, size_W=640)
        rain = img2tensor(rain_img, bgr2rgb=True, float32=True)
        gt = img2tensor(gt_img, bgr2rgb=True, float32=True)
...
class TestHIDE(data.Dataset):
    ...
    def get_images(self, index):
        ...
        rain_img = imfrombytes(read_img255(rain_path), float32=True)
        gt_img = imfrombytes(read_img255(gt_path), float32=True)
        [rain_img, gt_img] = align([rain_img, gt_img], size_H=720, size_W=1280)
        rain = img2tensor(rain_img, bgr2rgb=True, float32=True)
        gt = img2tensor(gt_img, bgr2rgb=True, float32=True)
...
class TestDPDD_dual(data.Dataset):
    def __init__(self, local_size, val_data_dir, flag=True):
        super().__init__()
        val_list_rain = "/home/jxy/projects_dir/datasets/DPDD/test.txt"
    ...
    def get_images(self, index):
        rain_name = self.rain_names[index]
        gt_name = self.gt_names[index]
        rain_path_L = os.path.join(self.val_data_dir, "inputL", rain_name)
        rain_path_R = os.path.join(self.val_data_dir, "inputR", rain_name)
        gt_path = os.path.join(self.val_data_dir, "target", gt_name)
        rain_img_R = imfrombytes(read_img255(rain_path_R), float32=True)
        rain_img_L = imfrombytes(read_img255(rain_path_L), float32=True)
        gt_img = imfrombytes(read_img255(gt_path), float32=True)
        h, w, c = rain_img_L.shape
        h = h - h % self.local_size
        w = w - w % self.local_size
        [rain_img_L, rain_img_R, gt_img] = align([rain_img_L, rain_img_R, gt_img], size_H=h, size_W=w)
        rain_L = img2tensor(rain_img_L, bgr2rgb=True, float32=True)
        rain_R = img2tensor(rain_img_R, bgr2rgb=True, float32=True)
        rain = torch.cat([rain_L, rain_R], dim=0)
        gt = img2tensor(gt_img, bgr2rgb=True, float32=True)
