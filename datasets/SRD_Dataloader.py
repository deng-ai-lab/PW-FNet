...
class TrainData(data.Dataset):
    def __init__(self, crop_size, train_data_dir, only_h_flip=False):
        super().__init__()
        train_list_rain = '/home/jxy/projects_dir/datasets/SRD/train.txt'

        with open(train_list_rain) as f:
            contents = f.readlines()
            gt_names = [i.strip() for i in contents]
            rain_names = [i.replace('_no_shadow', '') for i in gt_names]
    ...
    def get_images(self, index):
        ...
        rain_path = os.path.join(self.train_data_dir, 'shadow', rain_name)
        gt_path = os.path.join(self.train_data_dir, 'shadow_free', gt_name)
...
class TestData(data.Dataset):
    def __init__(self, local_size, val_data_dir, flag=True):
        ...
        val_list_rain = "/home/jxy/projects_dir/datasets/SRD/test.txt"

        with open(val_list_rain) as f:
            contents = f.readlines()
            gt_names = [i.strip() for i in contents]
            rain_names = [i.replace('_free', '') for i in gt_names]
    ...
    def get_images(self, index):
        ...
        rain_path = os.path.join(self.val_data_dir, 'test', 'shadow', rain_name)
        gt_path = os.path.join(self.val_data_dir, 'test', 'shadow_free', gt_name)
        # mask_path = os.path.join(self.val_data_dir, 'SRD_testmask', mask_name)
        rain_img = imfrombytes(read_img255(rain_path), float32=True)
        gt_img = imfrombytes(read_img255(gt_path), float32=True)
        # mask_img = imfrombytes(read_img255(mask_path), float32=True)
