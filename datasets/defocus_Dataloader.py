...
class TrainData(data.Dataset):
    def __init__(self, crop_size, only_h_flip=False):
        super().__init__()
        train_list_rain = '/home/jxy/projects_dir/datasets/LFDOF/train_data/train.txt'

        with open(train_list_rain) as f:
            contents = f.readlines()
            rain_names = [i.strip() for i in contents]
            gt_names = [i.split('input')[0] + 'ground_truth/' + i.split('/')[8] + '.png' for i in rain_names]
...
class TestData(data.Dataset):
    def __init__(self, flag=True):
        ...
        val_list_haze = '/home/jxy/projects_dir/datasets/LFDOF/test_data/test.txt'

        with open(val_list_haze) as f:
            contents = f.readlines()
            haze_names = [i.strip() for i in contents]
            gt_names = [i.split('input')[0] + 'ground_truth/' + i.split('/')[8] + '.png' for i in haze_names]
    ...
    def get_images(self, index):
        ...
        rain_img = imfrombytes(read_img255(rain_path), float32=True)
        gt_img = imfrombytes(read_img255(gt_path), float32=True)
        # [rain_img, gt_img] = align([rain_img, gt_img], size_H=1008, size_W=688)
        rain = img2tensor(rain_img, bgr2rgb=True, float32=True)
        gt = img2tensor(gt_img, bgr2rgb=True, float32=True)
