...
class TrainData_SPA(data.Dataset):
    def __init__(self, crop_size, train_data_dir, only_h_flip=False):
        super().__init__()
        train_list_rain = '/home/jxy/projects_dir/datasets/SPA/train_rain/LIST.TXT'
    ...
    def get_images(self, index):
        ...
        rain_path = os.path.join(self.train_data_dir, 'train_rain', rain_name)
        gt_path = os.path.join(self.train_data_dir, 'train_norain', gt_name)
...
class TestData_SPA(data.Dataset):
    def __init__(self, local_size, val_data_dir, flag=True):
        super().__init__()

        val_list_rain = "/home/jxy/projects_dir/datasets/SPA/test/rain/LIST.TXT"
    ...
    def get_images(self, index):
        ...
        rain_path = os.path.join(self.val_data_dir, 'test', "rain", rain_name)
        gt_path = os.path.join(self.val_data_dir, 'test', "norain", gt_name)
        rain_img = imfrombytes(read_img255(rain_path), float32=True)
        gt_img = imfrombytes(read_img255(gt_path), float32=True)
...
class TestData_GT_RAIN(data.Dataset):
    def __init__(self, local_size, val_data_dir, flag=True):
        super().__init__()

        val_list_rain = "/media/group2/jxy/datasets/gt_rain/GT_Rain_rainy.txt"

        with open(val_list_rain) as f:
            contents = f.readlines()
            rain_names = [i.strip() for i in contents]
            gt_names = [i.split("-R-")[0] + '-C-000.png' for i in rain_names]
    ...
    def get_images(self, index):
        ...
        rain_path = os.path.join(rain_name)
        gt_path = os.path.join(gt_name)
        rain_img = imfrombytes(read_img255(rain_path), float32=True)
        gt_img = imfrombytes(read_img255(gt_path), float32=True)
        h, w, c = rain_img.shape
        h = h - h % self.local_size
...
class TestData_LHP_Rain(data.Dataset):
    def __init__(self, local_size, val_data_dir, flag=True):
        super().__init__()

        val_list_rain = "/media/group2/jxy/datasets/gt_rain/LHP_rain.txt"
...
class TestData_rainy_in_driving(data.Dataset):
    def __init__(self, local_size, val_data_dir, flag=True):
        super().__init__()

        val_list_rain = "/home/jxy/projects_dir/datasets/gt_rain/rainy_in_driving.txt"
    ...
    def get_images(self, index):
        ...
        h = h - h % self.local_size
        w = w - w % self.local_size
        [rain_img] = align([rain_img], size_H=h, size_W=w)
        rain = img2tensor(rain_img, bgr2rgb=True, float32=True)
