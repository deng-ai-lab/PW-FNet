...
class ValData(data.Dataset):
    def __init__(self, crop_size_H, crop_size_W, val_data_dir, flag=True):
        ...
        val_list_haze = '/home/jxy/projects_dir/datasets/LSUI_new/train/train_hazy_list_379.txt'
    ...
    def get_images(self, index):
        ...
        haze_img = read_img255(haze_path)
        gt_img = read_img255(gt_path)
        [haze_img, gt_img] = align_for_valid([haze_img, gt_img], size_H=self.size_H, size_W=self.size_W)
        haze_img = np.ascontiguousarray(haze_img).astype('uint8')
        gt_img = np.ascontiguousarray(gt_img).astype('uint8')
        transform_haze = Compose([ToTensor()])
        transform_gt = Compose([ToTensor()])
...
class TestData(data.Dataset):
    def __init__(self, local_size, val_data_dir, flag=True):
        ...
        val_list_haze = '/home/jxy/projects_dir/datasets/LSUI_new/test/test_gt_list.TXT'
    ...
    def get_images(self, index):
        ...
        [haze_img, gt_img] = align_for_test([haze_img, gt_img], self.local_size)
        haze_img = np.ascontiguousarray(haze_img).astype('uint8')
        gt_img = np.ascontiguousarray(gt_img).astype('uint8')
        transform_haze = Compose([ToTensor()])
        transform_gt = Compose([ToTensor()])
