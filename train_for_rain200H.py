...
def train(train_loader, network, criterion, optimizer, alpha):
    losses = AverageMeter()
...
if __name__ == '__main__':
    ...
    # network.load_state_dict(torch.load('/home/jxy/projects_dir/PW-FNet/saved_models/rain/Backbone_mimo_iccv_rain200h_8448.pth')['state_dict'])
    ...
    if not os.path.exists(os.path.join(save_dir, args.model + test_str + '.pth')):
        ...
        for epoch in tqdm(range(setting['epochs'] + 1)):
            train_loss = train(train_loader, network, criterion, optimizer, epoch/setting['epochs'])
